...
public class ProcessLogs_MapperKeys {
	...
	public static void main(String[] args) throws IOException {
		...
		for (File container : logLocation.listFiles()) {
			...
			if (allMappers[mapperid] == null)
				allMappers[mapperid] = new Mapper(mapperid);
			
			fillMapperKeysRead (mapperid, syslog);
...
//		writeStatsReducersIndex();
		writeStatsMapperIndex();
	}

	private static void fillMapperKeysRead(int mapperid, File syslog
		...
		while (logReader.hasNextLine()) {
			...
			if (mapperEntry.contains("FATAL") &&
				...
				allMappers[mapperid].keys.add(value);
			}
		}
		logReader.close();
...
//							"null IPAddress",
	...
	private static void writeStatsMapperIndex() throws IOException {
		PrintWriter pr = new PrintWriter(
						new BufferedWriter(
