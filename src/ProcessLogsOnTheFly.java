...
public class ProcessLogsOnTheFly {

	static String pathPrefix = "/memex/akshay/project/hadoop-runtime/logs/";
	static String jobID;
	...
	public static void main(String[] args) throws IOException {
		...
		/** Read Mapper Logs and find the run-times */
 		String logLocationName = String.format(pathPrefix +
 											   "userlogs/application_%s",
 											   jobID);
		...
		for (File container : logLocation.listFiles()) {
			...
			Mapper mapper = new Mapper (mapperid);
			
			/* Fill in the keys processed/read by this mapper */
			fillMapperKeysRead (mapper, syslog);
			
			String value =
					processMapperEntry(getMapperEntry(syslog, "RunTimeTaken1"));
			mapper.runTime = Long.parseLong(value);

			value = processMapperEntry(getMapperEntry(syslog, "NextKeyValueTime"));
			mapper.nextKeyTime1 = Long.parseLong(value);
			
			value = processMapperEntry(getMapperEntry(syslog, "MapTime"));
			mapper.mapTime1 = Long.parseLong(value);
			
			value = processMapperEntry(getMapperEntry(syslog, "ReadTimeTaken"));
			mapper.nextKeyTime2 = Long.parseLong(value);
			
			value = processMapperEntry(getMapperEntry(syslog, "RunTimeTaken2"));
			mapper.mapTime2 = Long.parseLong(value);
			...
			for (int i = 0; i < numDataNodes; i++) {
				...
				Scanner dnLogReader =
					new Scanner (
					new File(pathPrefix + logFileName));
				processDNClientTrace(mapper, dnLogReader);
				dnLogReader.close();
			}
			slaveReader.close();
//			writeStatsCSV(mapper);
...
//		pr.close();
	...
	private static void fillMapperKeysRead(Mapper mapper, File syslog
		...
		while (logReader.hasNextLine()) {
			...
			if (mapperEntry.contains("FATAL") &&
				...
				mapperEntry.contains("KeyRead")) {
				String value = processMapperEntry (mapperEntry);
				mapper.keys.add(value);
			}
		}
		logReader.close();
	...
	private static String processMapperEntry(String mapperEntry) {
		...
		String value = "";
		if (mapperEntry.contains("KeyRead")) {
			parts = parts[1].split(":");
			for (int i = 1; i < parts.length-1; i++)
				value += parts[i].trim() + ":";
			value += parts[parts.length - 1].trim();
		}
		else value = parts[1].split(":")[1].trim();
		...
		while (dnLogReader.hasNextLine()) {
			...
			if (str.contains(clientTraceIdentifier)) {
				...
				if (str.contains(jobID + "_m_")) {
					...
					if (mapperid == -1 || mapperid != mapper.id) continue;
					
					mapper.blockids.add(extractPlainBlockID(ct.blockID));
...
//			this.ipaddress = null;
	...
	private static void writeStatsCSV(Mapper mapper) throws IOException {
		PrintWriter pr = new PrintWriter(
						new BufferedWriter(
						new FileWriter(outputFileName + ".xls", true)));
	...
	private static String extractPlainBlockID (String blockid) {
		if (blockid == null) {
			System.err.println ("NULL Block received.");
			return null;
		}
		String parts[] = blockid.split("_");
		if (parts.length != 3) {
			System.err.println ("Invalid block received.");
			return null;
		}
		return parts[1];
	}
	...
	private static void writeStatsMapperIndex(Mapper mapper)
		...
		String MAPPER_FORMAT = "%s %s\n";
		
		for (String key : mapper.keys) {
			pr.printf (MAPPER_FORMAT,
						key,
						mapper.blockids.toString()
					);
		}
		pr.close();
	}
